shadow$provide.module$node_modules$rlp$node_modules$bn_js$lib$bn=function(global,require,module$jscomp$0,exports$jscomp$0){(function(module,exports){function assert(val,msg){if(!val)throw Error(msg||"Assertion failed");}function inherits(ctor,superCtor){ctor.super_=superCtor;var TempCtor=function(){};TempCtor.prototype=superCtor.prototype;ctor.prototype=new TempCtor;ctor.prototype.constructor=ctor}function BN(number,base,endian){if(BN.isBN(number))return number;this.negative=0;this.words=null;this.length=
0;this.red=null;if(null!==number){if("le"===base||"be"===base)endian=base,base=10;this._init(number||0,base||10,endian||"be")}}function parseHex4Bits(string,index){index=string.charCodeAt(index);if(48<=index&&57>=index)return index-48;if(65<=index&&70>=index)return index-55;if(97<=index&&102>=index)return index-87;assert(!1,"Invalid character in "+string)}function parseHexByte(string,lowerBound,index){var r=parseHex4Bits(string,index);index-1>=lowerBound&&(r|=parseHex4Bits(string,index-1)<<4);return r}
function parseBase(str,start,end,mul){var r=0;end=Math.min(str.length,end);for(var i=start;i<end;i++){var c=str.charCodeAt(i)-48;r*=mul;start=49<=c?c-49+10:17<=c?c-17+10:c;assert(0<=c&&start<mul,"Invalid character");r+=start}return r}function move(dest,src){dest.words=src.words;dest.length=src.length;dest.negative=src.negative;dest.red=src.red}function inspect(){return(this.red?"\x3cBN-R: ":"\x3cBN: ")+this.toString(16)+"\x3e"}function smallMulTo(self,num,out){out.negative=num.negative^self.negative;
var len=self.length+num.length|0;out.length=len;len=len-1|0;var a=self.words[0]|0,b=num.words[0]|0;a*=b;var carry=a/67108864|0;out.words[0]=a&67108863;for(var k=1;k<len;k++){var ncarry=carry>>>26,rword=carry&67108863;carry=Math.min(k,num.length-1);for(var j=Math.max(0,k-self.length+1);j<=carry;j++)a=self.words[k-j|0]|0,b=num.words[j]|0,a=a*b+rword,ncarry+=a/67108864|0,rword=a&67108863;out.words[k]=rword|0;carry=ncarry|0}0!==carry?out.words[k]=carry|0:out.length--;return out._strip()}function bigMulTo(self,
...
//# sourceMappingURL=module$node_modules$rlp$node_modules$bn_js$lib$bn.js.map
