...
index-1)<<4);return r}function parseBase(str,start,end,mul){var r=0;end=Math.min(str.length,end);for(var i=start;i<end;i++){var c=str.charCodeAt(i)-48;r*=mul;start=49<=c?c-49+10:17<=c?c-17+10:c;assert(0<=c&&start<mul,"Invalid character");r+=start}return r}function move(dest,src){dest.words=src.words;dest.length=src.length;dest.negative=src.negative;dest.red=src.red}function inspect(){return(this.red?"\x3cBN-R: ":"\x3cBN: ")+this.toString(16)+"\x3e"}function smallMulTo(self,num,out){out.negative=num.negative^
self.negative;var len=self.length+num.length|0;out.length=len;len=len-1|0;var a=self.words[0]|0,b=num.words[0]|0;a*=b;var carry=a/67108864|0;out.words[0]=a&67108863;for(var k=1;k<len;k++){var ncarry=carry>>>26,rword=carry&67108863;carry=Math.min(k,num.length-1);for(var j=Math.max(0,k-self.length+1);j<=carry;j++)a=self.words[k-j|0]|0,b=num.words[j]|0,a=a*b+rword,ncarry+=a/67108864|0,rword=a&67108863;out.words[k]=rword|0;carry=ncarry|0}0!==carry?out.words[k]=carry|0:out.length--;return out._strip()}
...
//# sourceMappingURL=module$node_modules$browserify_sign$node_modules$bn_js$lib$bn.js.map
