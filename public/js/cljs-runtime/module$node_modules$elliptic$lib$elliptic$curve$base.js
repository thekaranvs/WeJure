...
Array(4);this._bitLength=this.n?this.n.bitLength():0;type=this.n&&this.p.div(this.n);!type||0<type.cmpn(100)?this.redN=null:(this._maxwellTrick=!0,this.redN=this.n.toRed(this.red))}function BasePoint(curve,type){this.curve=curve;this.type=type;this.precomputed=null}var BN=require("module$node_modules$bn_DOT_js$lib$bn"),utils=require("module$node_modules$elliptic$lib$elliptic$utils"),getNAF=utils.getNAF,getJSF=utils.getJSF,assert=utils.assert;module.exports=BaseCurve;BaseCurve.prototype.point=function(){throw Error("Not implemented");
};BaseCurve.prototype.validate=function(){throw Error("Not implemented");};BaseCurve.prototype._fixedNafMul=function(p,k){assert(p.precomputed);p=p._getDoubles();var naf=getNAF(k,1,this._bitLength),I=(1<<p.step+1)-(0===p.step%2?2:1);I/=3;k=[];var j;for(j=0;j<naf.length;j+=p.step){var nafW=0;for(var l=j+p.step-1;l>=j;l--)nafW=(nafW<<1)+naf[l];k.push(nafW)}naf=this.jpoint(null,null,null);for(l=this.jpoint(null,null,null);0<I;I--){for(j=0;j<k.length;j++)nafW=k[j],nafW===I?l=l.mixedAdd(p.points[j]):nafW===
-I&&(l=l.mixedAdd(p.points[j].neg()));naf=naf.add(l)}return naf.toP()};BaseCurve.prototype._wnafMul=function(p,k){var nafPoints=p._getNAFPoints(4),wnd=nafPoints.points;k=getNAF(k,nafPoints.wnd,this._bitLength);nafPoints=this.jpoint(null,null,null);for(var i=k.length-1;0<=i;i--){for(var l=0;0<=i&&0===k[i];i--)l++;0<=i&&l++;nafPoints=nafPoints.dblp(l);if(0>i)break;l=k[i];assert(0!==l);nafPoints="affine"===p.type?0<l?nafPoints.mixedAdd(wnd[l-1>>1]):nafPoints.mixedAdd(wnd[-l-1>>1].neg()):0<l?nafPoints.add(wnd[l-
1>>1]):nafPoints.add(wnd[-l-1>>1].neg())}return"affine"===p.type?nafPoints.toP():nafPoints};BaseCurve.prototype._wnafMulAdd=function(defW,points,coeffs,len,jacobianResult){var wndWidth=this._wnafT1,wnd=this._wnafT2,naf=this._wnafT3,max=0,i;for(i=0;i<len;i++){var p=points[i];var nafPoints=p._getNAFPoints(defW);wndWidth[i]=nafPoints.wnd;wnd[i]=nafPoints.points}for(i=len-1;1<=i;i-=2){nafPoints=i-1;var b=i;if(1!==wndWidth[nafPoints]||1!==wndWidth[b])naf[nafPoints]=getNAF(coeffs[nafPoints],wndWidth[nafPoints],
this._bitLength),naf[b]=getNAF(coeffs[b],wndWidth[b],this._bitLength),max=Math.max(naf[nafPoints].length,max),max=Math.max(naf[b].length,max);else{var comb=[points[nafPoints],null,null,points[b]];0===points[nafPoints].y.cmp(points[b].y)?(comb[1]=points[nafPoints].add(points[b]),comb[2]=points[nafPoints].toJ().mixedAdd(points[b].neg())):0===points[nafPoints].y.cmp(points[b].y.redNeg())?(comb[1]=points[nafPoints].toJ().mixedAdd(points[b]),comb[2]=points[nafPoints].add(points[b].neg())):(comb[1]=points[nafPoints].toJ().mixedAdd(points[b]),
